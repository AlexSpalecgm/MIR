...
archive_directory="/home/psmon/scheduler/bin/log/archive"  # Новая директория для архивов
...
if [ "$day_of_month" -eq 01 ]; then
    ...
    archive_name="${archive_directory}/logs_${prev_month}.tar.gz"

    # Находим все файлы за прошлый месяц
    files_to_archive=$(find "$log_directory" -type f \
        -name "log_*.txt" \
        -newermt "${prev_month}-01" ! -newermt "${month_year}-01")

    if [ -n "$files_to_archive" ]; then
        echo "$(date +"%Y-%m-%d %H:%M:%S") - INFO: Архивируем файлы за ${prev_month}..."
        # Создаем архив с сжатием
        tar -czf "$archive_name" -C "$log_directory" $(basename -a $files_to_archive)
        echo "$(date +"%Y-%m-%d %H:%M:%S") - INFO: Архив создан: $archive_name"
    else
        echo "$(date +"%Y-%m-%d %H:%M:%S") - INFO: Нет файлов за ${prev_month} для архивации."
    fi
else
    echo "$(date +"%Y-%m-%d %H:%M:%S") - INFO: Сегодня не 1-е число месяца. Скрипт не выполняется."
fi
...
for file in $files; do
    ...
    if (( 10#$hour >= 24 || 10#$minute >= 60 )); then
        echo "$(date +"%Y-%m-%d %H:%M:%S") - ERROR: $file" >> "$log_bad_file"
        rm "$file"
        continue
    fi
